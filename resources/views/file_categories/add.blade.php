...
@section('breadcrumbs')
	...
	<li class="active"><span><i class="fa fa-plus"></i> New File Category</span></li>
@stop
...
@section('content')
	<form method="post" action="{{route('admin-file_categories-add')}}" role="form" accept-charset="UTF-8" enctype="multipart/form-data">
		<div class="row">
			...
			<div class="col-sm-6 col-md-4 col-lg-4">
				<div class="form-group{!! $errors->has('max_size') ? ' has-error':'' !!}">
					...
					<input type="text" class="form-control" id="max_size" name="max_size" placeholder="Enter maximum file size"{!! ((old('max_size')) ? ' value="'.old('max_size').'"' : ' value="100"') !!}>
					{!! $errors->has('max_size') ? '<span class="text-danger">'.$errors->first('max_size').'</span>' : '' !!}
					<div class="help-block">Enter values in Mega Bytes</div>
				</div>
			</div>
			<div class="col-sm-6 col-md-8 col-lg-8">
				<div class="form-group{!! $errors->has('file_types') ? ' has-error':'' !!}">
					<label for="file_types">File Types</label>
					<input type="text" class="form-control" id="file_types" name="file_types" placeholder="Enter file types"{!! ((old('file_types')) ? ' value="'.old('file_types').'"' : ' value="pdf,doc,xls,xlsx,docx"') !!}>
