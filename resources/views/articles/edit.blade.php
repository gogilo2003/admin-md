...
@section('breadcrumbs')
    ...
    <li>
        <a href="{{ route('admin-articles') }}"><i class="fas fa-copy"></i> Articles</a>
    </li>
...
@section('content')
    <form method="post" action="{{ route('admin-articles-edit-post') }}" article="form" accept-charset="UTF-8"
        ...
        <div class="row">
            <div class="col-md-5 col-lg-3">
                    ...
                    <select class="selectpicker form-control" id="category" name="category" data-live-search="true"
                        ...
                        @foreach (Ogilo\AdminMd\Models\ArticleCategory::all() as $category)
                            <option value="{{ $category->id }}"
                                {{ old('category') ? (old('category') === $category->id ? 'selected' : '') : ($article->article_category_id === $category->id ? 'selected' : '') }}>
                ...
                <div class="form-group">
                    ...
                    <select class="selectpicker form-control" id="icon" name="icon" data-live-search="true"
                        data-size="5" data-style="btn btn-link">

                        @foreach (get_icons() as $icon => $caption)
                            <option value="{{ $icon }}"
                                data-icon="{{ $icon }}"{{ old('icon') ? (old('icon') == $icon ? 'selected' : '') : ($article->icon == $icon ? 'selected' : '') }}>
                                {{ $caption }}</option>
                        @endforeach
            ...
            <div class="col-md-7 col-lg-9">
                <div class="form-group{!! $errors->has('title') ? ' has-error' : '' !!}">
                    ...
                    <input type="text" class="form-control" id="title" name="title"
                        placeholder="Enter title"{!! old('title') ? ' value="' . old('title') . '"' : ' value="' . $article->title . '"' !!}>
        ...
        <div class="offset-md-5 offset-lg-3">
            ...
            <button type="submit" class="btn btn-primary btn-lg btn-round"><span class="fa fa-floppy-o"></span>
                Save</button>
        </div>
    </form>
@stop
