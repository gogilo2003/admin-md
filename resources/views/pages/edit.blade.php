...
@section('breadcrumbs')
    @parent
    <li>
        <a href="{{ route('admin-pages') }}"><i class="fas fa-copy"></i> Pages</a>
    </li>
...
@section('content')
    <form method="post" action="{{ route('admin-pages-edit-post') }}" page="form" accept-charset="UTF-8"
        ...
        <div class="row">
            <div class="col-md-5">
                <div class="form-group{!! $errors->has('name') ? ' has-error' : '' !!}">
                    ...
                    <div>
                        <img id="image_preview" class="image_preview"
                            src="{{ $page->title_image ? asset(config('admin.path_prefix') . 'images/pages/' . $page->title_image) : asset(config('admin.path_prefix') . 'vendor/admin/img/placeholder.png') }}"
                            class="img-responsive img-fluid" alt="Preview">
                    </div>
                    <input type="file" name="title_image" id="title_image">
                </div>

                <a href="JavaScript:" id="load_image" class="btn btn-round btn-primary btn-block"><i
                ...
                <div class="form-row">
                        ...
                        <select class="form-control selectpicker" id="link" name="link" data-style="btn btn-link">
                            @foreach (Ogilo\AdminMd\Models\Link::all() as $link)
                                <option value="{{ $link->id }}"
                                    {{ $page->isSelected(old('link') ? old('link') : $link->id) ? 'selected' : '' }}>
                                    {{ $link->caption }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-sm-6 col-md-8 col-lg-8 form-group{!! $errors->has('name') ? ' has-error' : '' !!}">
                        ...
                        <input type="text" readonly class="form-control" id="name" name="name"
                            placeholder="Enter name"{!! old('name') ? ' value="' . old('name') . '"' : ' value="' . $page->name . '"' !!}>
                    ...
                    <div class="col-md-12 form-group{!! $errors->has('title') ? ' has-error' : '' !!}">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                            placeholder="Enter value"{!! old('title') ? ' value="' . old('title') . '"' : ' value="' . $page->title . '"' !!}>
