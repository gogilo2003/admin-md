...
@section('breadcrumbs')
    @parent
    <li class="active"><span><i class="fas fa-copy"></i> Blogs</span></li>
@endsection
...
@section('content')
    <div>
        <a href="{{ route('admin-blogs-add') }}" class="btn btn-primary btn-sm"><span class="fa fa-plus"></span>&nbsp;&nbsp;
            New Blog</a>
        <a href="{{ route('admin-pages') }}" class="btn btn-primary btn-sm"><span class="fas fa-copy"></span>&nbsp;&nbsp;
            Pages</a>
    </div>
    <hr>
        ...
        <tbody>
            @foreach ($blogs as $blog)
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="{{ route('admin-blogs-edit', $blog->id) }}" class="btn btn-primary btn-sm"><span
                                    class="fa fa-edit"></span>&nbsp;&nbsp; Edit</a>
                            <a href="{{ route('admin-blogs-view', $blog->id) }}" class="btn btn-primary btn-sm"><span
                                    class="fa fa-list"></span>&nbsp;&nbsp; View</a>
                            <a href="javascript:" data-id="{{ $blog->id }}"
                                class="btn btn-primary btn-sm publishArticle"><span
                                    class="fa fa-arrow-{{ $blog->published ? 'down' : 'up' }}"></span>&nbsp;&nbsp;
                                {{ $blog->published ? 'Un-publish' : 'Publish' }}</a>
                            <a href="javascript:" data-id="{{ $blog->id }}"
                                class="btn btn-primary btn-sm featureArticle"><span
                                    class="fa fa-arrow-{{ $blog->featured ? 'down' : 'up' }}"></span>&nbsp;&nbsp;
                                {{ $blog->featured ? 'Un-feature' : 'Feature' }}</a>
                            <button data-tags="{{ json_encode($blog->tags->pluck('id')) }}" data-id="{{ $blog->id }}"
                                class="btn btn-info btn-sm" data-toggle="modal" href='#tagsModal'><i
                                    class="fas fa-tags"></i>&nbsp;Tags</button>
                            <a href="javascript:" data-id="{{ $blog->id }}"
                    ...
                    if (answer) {
                        ...
                        $.ajax({
                            ...
                            if (xhr.success) {
                                ...
                                icon.className = "fa fa-arrow-" + (xhr.published ? 'down' :
                                    'up')
                    ...
                    if (answer) {
                        ...
                        $.ajax({
                            ...
                            if (xhr.success) {
                                btn.innerHTML = null
                                let icon = document.createElement('i')
                                icon.className = "fa fa-arrow-" + (xhr.featured ? 'down' :
                                    'up')
                                btn.appendChild(icon)
