...
@section('content')
    ...
    <a href="{{ route('admin-pictures-add') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>&nbsp;&nbsp; Add
        Picture</a>
    <hr>
    @if ($pictures->count())
        <table id="picturesTable" class="table table-striped">
            ...
            @foreach ($pictures as $key => $picture)
                <tr>
                    ...
                    <td width="128">
                        <img src="{{ asset(config('admin.path_prefix') . 'images/pictures/thumbnails/' . $picture->name) }}"
                            class="img-responsive img-fluid" alt="{{ $picture->alt }}">
                    </td>
                    <td>
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="{{ route('admin-pictures-edit', $picture->id) }}"
                                class="btn btn-success btn-sm btn-round"><span class="fa fa-edit"></span>&nbsp;&nbsp;
                                Edit</a>
                            <a href="javascript:publishPicture({{ $picture->id }})"
                                class="btn btn-warning btn-sm btn-round"><span
                                    class="fa fa-arrow-{{ $picture->published ? 'down' : 'up' }}"></span>&nbsp;&nbsp;
                                {{ $picture->published ? ' Unpublish' : 'Publish' }}</a>
                            <a href="javascript:featurePicture({{ $picture->id }})"
                                class="btn btn-info btn-sm btn-round"><span
                                    class="fa fa-arrow-{{ $picture->featured ? 'down' : 'up' }}"></span>&nbsp;&nbsp;
                                {{ $picture->featured ? ' Unfeature' : 'Feature' }}</a>
                            <a href="javascript:deletePicture({{ $picture->id }})"
                                class="btn btn-danger btn-sm btn-round"><span class="fas fa-trash-alt"></span>&nbsp;&nbsp;
                                Delete</a>
                        </div>
                    </td>
                </tr>
