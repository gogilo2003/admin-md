...
@section('content')
    <a href="{{ route('admin-picture_categories-add') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>&nbsp; New
        Picture Category</a>
        ...
        <tbody>
            @foreach ($categories as $category)
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="{{ route('admin-picture_categories-edit', $category->id) }}"
                                class="btn btn-primary btn-sm"><i class="fa fa-edit"></i>&nbsp;Edit</a>
                            <a href="javascript:" class="btn btn-danger btn-sm deletePictureCategory"
                                data-title="{{ $category->title }}" data-id="{{ $category->id }}"><span
                                    class="fas fa-trash-alt"></span>&nbsp; Delete</a>
                            <a data-pages="{{ json_encode($category->pageIds()) }}" data-id="{{ $category->id }}"
                                class="btn btn-primary btn-sm" data-toggle="modal" href='#pagesModal'><i
    ...
    <div class="modal fade" id="pagesModal" data-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{ route('admin-picture_categories-pages') }}" role="form"
                    accept-charset="UTF-8" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <button type="button" class="close btn btn-fab btn-danger btn-round btn-sm"
                                data-dismiss="modal" aria-hidden="true" style><span
                                    class="material-icons">close</span></button>
                            <h4 class="card-title">Select Pages</h4>
                        </div>
                        <div class="card-body">
                            <select name="pages[]" id="pages" class="selectpicker" data-tick-icon="fa fa-check-square"
                                multiple data-width="100%" data-size="5" data-style="btn btn-link">
                        ...
                        <div class="card-footer">
                            <button type="button" class="btn btn-danger btn-round" data-dismiss="modal"><span
